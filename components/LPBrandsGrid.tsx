...
export default function LPBrandsGrid({ variant = "8x4", label }: LPBrandsGridProps) {
  const columns = variant === "8x4" ? 8 : 5;
  const rows = 4;
  const gridBrands = brands.slice(0, columns * rows);

  const handleSubscriptionClick = () => {
    window.location.href = 'https://platform.sportendorse.com/signup/brand?subscription=trial';
  };
